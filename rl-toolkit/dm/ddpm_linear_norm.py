...
class MLPDiffusion(nn.Module):
    def __init__(self, n_steps, input_dim=6, num_units=128, depth=5, device='cuda'):
...
def diffusion_loss_fn(model, x_0, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, n_steps):
    ...
    # t = torch.randint(1, n_steps+1, size=(batch_size//2,)).to(device)
    # t = torch.cat([t, n_steps-t], dim=0) #[batch_size, 1]
    t = t.unsqueeze(-1)
    # coefficient of x0
    a = alphas_bar_sqrt[t]
    # coefficient of eps
    ...
    for t in tqdm(range(0, num_epoch)):
        ...
        if t % 500 == 0:
            ...
            with torch.no_grad():
                ...
                plt.savefig(f'rl-toolkit/dm/trained_imgs/{env}-pos_{args.lr}_{args.hidden_dim}_{args.num_epoch}_{args.scheduler_type}_norm.png')
                plt.close()
        if t % 20 == 0:
            ...
            plt.savefig(f'rl-toolkit/dm/trained_imgs/{env}_ddpm_loss_{args.lr}_{args.hidden_dim}_{args.num_epoch}_{args.scheduler_type}_norm.png')
            plt.close()    
        if t % 1000 == 0:
            torch.save(model.state_dict(), f'rl-toolkit/dm/trained_models/{env}_ddpm_{args.lr}_{args.hidden_dim}_{args.num_epoch}_{args.scheduler_type}_norm.pt')
    torch.save(model.state_dict(), f'rl-toolkit/dm/trained_models/{env}_ddpm_{args.lr}_{args.hidden_dim}_{args.num_epoch}_{args.scheduler_type}_norm.pt')