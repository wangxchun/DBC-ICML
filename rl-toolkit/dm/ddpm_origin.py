    ...
    for t in tqdm(range(0, num_epoch)):
        ...
        if t % 500 == 0:
            ...
            with torch.no_grad():
                ...
                plt.savefig(f'{env}-pos-fix-origin.png')
                plt.close()
        if t % 20 == 0:
            ...
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            plt.title(env + '_ddpm_loss_origin')
            plt.savefig(env + '_ddpm_loss_origin.png')
            plt.close()    
        if t % 1000 == 0:
            torch.save(model.state_dict(), f'rl-toolkit/dm/trained_models/{env}_ddpm_origin_.pt')
    torch.save(model.state_dict(), f'rl-toolkit/dm/trained_models/{env}_ddpm_origin_.pt')
