    ...
    for t in tqdm(range(0, num_epoch)):
        ...
        if t % 500 == 0:
            ...
            with torch.no_grad():
                ...
                plt.savefig(f'rl-toolkit/dm/trained_imgs/{env}-pos_{args.lr}_{args.scheduler_type}.png')
                plt.close()
        if t % 20 == 0:
            ...
            plt.savefig(f'rl-toolkit/dm/trained_imgs/{env}_ddpm_loss_{args.lr}_{args.scheduler_type}.png')
            plt.close()    
        if t % 1000 == 0:
            torch.save(model.state_dict(), f'rl-toolkit/dm/trained_models/{env}_ddpm_{args.lr}_{args.scheduler_type}.pt')
    torch.save(model.state_dict(), f'rl-toolkit/dm/trained_models/{env}_ddpm_{args.lr}_{args.scheduler_type}.pt')